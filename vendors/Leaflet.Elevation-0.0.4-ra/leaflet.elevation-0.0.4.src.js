...
L.Control.Elevation = L.Control.extend({
    ...
    _addGPXdata: function (coords) {
        ...
        if (coords) {
            ...
            for (var i = 0; i < coords.length; i++) {
                ...
                var newdist = opts.imperial ? s.distanceTo(e) * this.__mileFactor : s.distanceTo(e);
                dist = dist + Math.round(newdist / 1000 * 100000) / 100000;
                if (typeof (s.meta) === "undefined") {
                    elev = s.alt;
                } else {
                    elev = s.meta.ele;
                }
                ele = ele < elev ? elev : ele;
                data.push({
                    dist: dist,
                    altitude: opts.imperial ? elev * this.__footFactor : elev,
                    x: s.lng,
                    y: s.lat,
                    latlng: s
                });
    ...
    _applyData: function () {
        ...
        if (Math.abs(ydomain[0] - ydomain[1]) < 5) { // if elevations as all equal 
            ydomain[0] -= 5;
            ydomain[1] += 5;
        }
