...
class RJsonwalksBU51Inline extends RJsonwalksDisplaybase {
    ...
    public function DisplayWalks($walks) {
        ...
        foreach ($items as $walk) {
            echo '<div  id=W' . $walk->id . ' class="ra-accordion-item ' . $this->walkClass . $walk->status . '" itemscope itemtype="http://schema.org/Event">' . PHP_EOL;
            echo '<a itemprop=url href="#W' .  $walk->id . '" /></a>'. PHP_EOL;
            if ($this->printOn) {
    ...
    private function displayWalkSummary($walk) {
	    ...
	    $text .= "<b  itemprop=name> " . $walk->title . "</b> ";    
        $text .= "<br /> " . $walk->walkDate->format('l, jS F Y');
	    if ($walk->hasMeetPlace) {
            $text .= " meet at " . $walk->meetLocation->timeHHMMshort . " ";
        }
	    if ($walk->startLocation->exact) {
            $text .= " start at <meta itemprop=startDate content=\"" . $walk->startLocation->time->format(DateTime::ISO8601) . "\" /> " . $walk->startLocation->timeHHMMshort ;
        }
        if ($walk->finishTime != null) {
            $text .= " estimated finish at <meta itemprop=endDate content=\"" . $walk->finishTime->format(DateTime::ISO8601) . "\" /> " .  $this->getShortTime($walk->finishTime);
        }   
    ...
    function displayWalkDetails($walk) {
       ...
       if ($walk->groupCode != $homeGroupCode ) {
        ...
        if ($walk->description != "") {
            echo "<div class='description' itemprop=description> " . $walk->descriptionHtml . "</div>" . PHP_EOL;
        }
	echo "</div>";
        ...
        if ($walk->startLocation->exact) {
            echo "<div class='startplace' itemprop='location' itemscope itemtype='http://schema.org/Place'>";
        } else {
    ...
    private
        ...
        if ($location->exact) {
            ...
            $out.= RHtml::withDiv("latlong" , "<b>Latitude</b>: <span itemprop='latitude'>" . $location->latitude . "</span> , <b>Longitude</b>: <span itemprop='longitude'>" . $location->longitude . "</span>", $this->printOn, "itemprop=geo itemscope itemtype='http://schema.org/GeoCoordinates'");
